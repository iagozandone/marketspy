...
// Inicializa o app Express no escopo global do arquivo para a Vercel Serverless ler
const app = express();
...
app.post("/api/ml/refresh-token", async (req, res) => {
  try {
    ...
    console.error("[ML Refresh Error]", error?.response?.data || error);
...
app.get("/api/test-ml", async (_req, res) => {
  try {
    ...
    console.error("[ML TEST ERROR]", error?.response?.data || error);
    return res.status(500).json({
      success: false,
...
// Suporte para rodar localmente (npm run dev / start)
...
// Exportação crucial para a Vercel ligar os motores Serverless nesta rota
export default app;